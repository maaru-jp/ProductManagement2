...
(function (global) {
  ...
  var MYSHIP_RUNTIME_HELPERS = [
    ...
    "}",
    "",
    "function getNamedFieldScope() {",
    "  const form = getMainForm();",
    "  const formCount = form ? getAllNamedFields(form, true).length : 0;",
    "  const docCount = getAllNamedFields(document, true).length;",
    "  if (docCount > formCount + 2) return document;",
    "  return form || document;",
    "}",
    "",
    "function getAllNamedFields(root, raw) {",
    "  const scope = raw ? (root || document) : (root || getNamedFieldScope());",
    "  return Array.from(scope.querySelectorAll(\"input[name], textarea[name], select[name]\"));",
    "}",
    "",
    "function findNamedField(name, root) {",
    "  return getAllNamedFields(root || document, true).find((el) => el.name === name) || null;",
    "}",
    "",
    ...
    "}",
    "",
    "function getProductIndexedGroup(root) {",
    "  const names = getAllNamedFields(root || document, true).map((el) => el.name).filter(Boolean);",
    "  const groups = {};",
    "  names.forEach((n) => {",
    "    const m = n.match(/^(.+?)\\[0\\](\\..+)$/);",
    "    if (!m) return;",
    ...
    "  });",
    "  const ranked = Object.entries(groups).map(([prefix, samples]) => {",
    "    const kinds = new Set(samples.map((s) => classifyField(s)).filter(Boolean));",
    "    let score = samples.length;",
    "    if (kinds.has(\"name\")) score += 60;",
    "    if (kinds.has(\"price\")) score += 60;",
    "    if (kinds.has(\"qty\")) score += 30;",
    "    if (/detail|goods|product|item|case/i.test(prefix)) score += 20;",
    "    return { prefix, samples, score };",
    "  }).filter((g) => g.score >= 60).sort((a, b) => b.score - a.score);",
    "  return ranked[0] || null;",
    "}",
    "",
    "function getIndexedSampleFields(root) {",
    "  const group = getProductIndexedGroup(root);",
    ...
    "  const scope = root || getNamedFieldScope();",
    "  const allNames = getAllNamedFields(scope, true).map((el) => el.name).filter(Boolean);",
    "  const group = getProductIndexedGroup(scope);",
    "  if (group) return { mode: \"indexed\", prefix: group.prefix, samples: group.samples, allNames };",
    "  const flat = discoverFlatProductFields(scope);",
    "  if (flat.name && flat.price) return { mode: \"flat\", flat, allNames };",
    "  return { mode: \"dom\", allNames };",
    "}",
    "",
    "function appendStoreItems(fd, plan, store) {",
    ...
    "        const templateEl = findNamedField(sample);",
    ...
    "        else if (templateEl && templateEl.type !== \"file\") fd.append(field, templateEl.value);",
    "        used.add(field);",
    "      });",
    "    });",
    "    return;",
    ...
    "}",
    "",
    "function setStoreName(fd, store) {",
    "  const storeKeys = [\"StoreName\", \"storeName\", \"ShopName\", \"MarketName\", \"Title\", \"CaseName\"];",
    "  let storeSet = false;",
    "  storeKeys.forEach((key) => {",
    "    if (findNamedField(key)) {",
    "      fd.set(key, store.name);",
    "      storeSet = true;",
    "    }",
    "  });",
    "  if (!storeSet) {",
    "    const storeEl = getAllNamedFields(document, true).find((el) => {",
    ...
    "  if (action) return new URL(action, location.origin).href;",
    "  return MYSHIP_FAST_URL;",
    "}",
    "",
    "function buildFD(store, plan) {",
    "  const scope = getNamedFieldScope();",
    "  if (!scope) throw new Error(\"找不到表單。請在 \" + MYSHIP_FAST_URL + \" 頁面執行（須已登入）\");",
    "  plan = plan || discoverProductPlan(scope);",
    "  if (plan.mode === \"dom\") throw new Error(\"DOM_ONLY\");",
    "  syncTokenFromDom();",
    "  const productPrefix = plan.prefix || \"\";",
    "  const fd = new FormData();",
    "  getAllNamedFields(scope, true).forEach((el) => {",
    "    if (el.name === \"__RequestVerificationToken\") return;",
    "    if (!el.name) return;",
    "    if (productPrefix && el.name.indexOf(productPrefix + \"[\") === 0) return;",
    "    if (!productPrefix && isIndexedField(el.name)) return;",
    ...
    "  setStoreName(fd, store);",
    "  if (plan.mode !== \"indexed\" && plan.mode !== \"flat\") {",
    "    const hint = (plan.allNames || []).slice(0, 12).join(\", \");",
    "    throw new Error(\"找不到商品欄位。請先點「新增商品」後執行 __maaruMyshipProbe()。目前欄位：\" + (hint || \"（無 name 欄位）\"));",
    "  }",
    "  appendStoreItems(fd, plan, store);",
    "  return fd;",
    "}",
    "",
    "function parseMyshipError(text) {",
    "  const html = String(text || \"\");",
    "  const codeM = html.match(/Code[：:]\\s*(\\d+)/);",
    "  if (html.includes(\"系統忙碌\") || (codeM && codeM[1] === \"120\")) {",
    "    return \"賣貨便回傳：系統忙碌（Code 120）。通常是表單欄位不符或請求過快，腳本會改以頁面「上架」送出。\";",
    "  }",
    "  if (codeM) return \"賣貨便錯誤 Code \" + codeM[1];",
    "  if (/查詢資料失敗|資料不存在|錯誤|失敗/.test(html) && html.length < 8000) return \"賣貨便回傳錯誤頁\";",
    "  return \"\";",
    "}",
    "",
    "function extractGmId(text, url) {",
    "  const bag = String(text || \"\") + \" \" + String(url || \"\");",
    "  const m = bag.match(/GM[A-Za-z0-9]+/);",
    "  return m ? m[0] : \"\";",
    "}",
    "",
    ...
    "  const nodes = Array.from(document.querySelectorAll(\"label, span, div, th, p, b, strong, h6\"));",
    "  for (const node of nodes) {",
    "    const t = norm(node.textContent || \"\");",
    "    if (!t || t.length > 24) continue;",
    "    if (!t.includes(want)) continue;",
    "    const box = node.closest(\"tr, .form-group, .row, .field, [class*='col'], .mb-3, .mb-2, .card-body, section\") || node.parentElement;",
    "    if (!box) continue;",
    "    const inp = box.querySelector(\"input:not([type=hidden]):not([type=file]), textarea\");",
    "    if (inp) return inp;",
    "  }",
    "  return null;",
    "}",
    "",
    "async function clickAddProductIfNeeded(itemIndex) {",
    "  if (itemIndex <= 0) return;",
    "  const clickables = Array.from(document.querySelectorAll(\"button, a, [role='button'], .btn, input[type='button'], span\"));",
    "  const addBtn = clickables.find((el) => /繼續新增|新增商品|加入商品/.test(norm(el.textContent || el.value || \"\")));",
    "  if (addBtn) { addBtn.click(); await sleep(700); }",
    "}",
    "",
    "async function fillStoreDom(store, plan) {",
    "  plan = plan || discoverProductPlan();",
    "  const storeKeys = [\"StoreName\", \"storeName\", \"ShopName\", \"MarketName\", \"Title\", \"CaseName\"];",
    "  let storeSet = false;",
    "  for (const key of storeKeys) {",
    "    const el = findNamedField(key);",
    "    if (el) { setNativeInput(el, store.name); storeSet = true; break; }",
    "  }",
    "  if (!storeSet) {",
    "    const storeEl = findInputNearLabel(\"賣場名稱\") || getAllNamedFields(document, true).find((el) => {",
    "      const bag = norm(el.name + el.id + (el.placeholder || \"\"));",
    "      return bag.includes(\"賣場\") && bag.includes(\"名稱\");",
    "    });",
    "    if (storeEl) setNativeInput(storeEl, store.name);",
    "  }",
    "  for (let i = 0; i < store.items.length; i++) {",
    "    await clickAddProductIfNeeded(i);",
    "    const item = store.items[i];",
    "    if (plan.mode === \"indexed\") {",
    "      (plan.samples || []).forEach((sample) => {",
    "        const field = sample.replace(/\\[0\\]/g, \"[\" + i + \"]\");",
    "        const el = findNamedField(field);",
    "        if (!el) return;",
    "        const kind = classifyField(field);",
    "        if (kind === \"name\") setNativeInput(el, item.name);",
    "        else if (kind === \"desc\") setNativeInput(el, item.desc || item.name);",
    "        else if (kind === \"price\") setNativeInput(el, String(item.price));",
    "        else if (kind === \"qty\") setNativeInput(el, String(item.qty));",
    "        else if (kind === \"stock\") setNativeInput(el, String(Math.max(item.qty, 99)));",
    "      });",
    "    } else {",
    "      const nameEl = findInputNearLabel(\"商品名稱\") || findInputNearLabel(\"品名\");",
    "      const priceEl = findInputNearLabel(\"售價\") || findInputNearLabel(\"金額\");",
    "      const qtyEl = findInputNearLabel(\"數量\");",
    "      const descEl = findInputNearLabel(\"描述\") || findInputNearLabel(\"介紹\");",
    "      if (nameEl) setNativeInput(nameEl, item.name);",
    "      if (descEl) setNativeInput(descEl, item.desc || item.name);",
    "      if (priceEl) setNativeInput(priceEl, String(item.price));",
    "      if (qtyEl) setNativeInput(qtyEl, String(item.qty));",
    "    }",
    "  }",
    "}",
    "",
    ...
    "    return t.includes(\"上架\") && !t.includes(\"預覽\");",
    "  });",
    "  if (!btn) throw new Error(\"找不到「上架」按鈕\");",
    "  btn.click();",
    "  await sleep(2200);",
    "}",
    "",
    "async function submitStoreDom(store, plan) {",
    "  await ensureProductRowVisible();",
    "  await fillStoreDom(store, plan);",
    "  await clickPublishButton();",
    "  const id = extractGmId(document.body ? document.body.innerHTML : \"\", location.href);",
    "  if (!id) throw new Error(\"DOM 上架後未取得 GM 賣場編號，請查看頁面是否成功\");",
    "  return id;",
    "}",
    "",
    "async function postStoreFetch(store, plan) {",
    "  const resp = await fetch(getPostUrl(), {",
    "    method: \"POST\",",
    "    body: buildFD(store, plan),",
    "    redirect: \"follow\",",
    "    credentials: \"include\",",
    "    headers: {",
    ...
    "  const nextToken = extractTokenFromHtml(text);",
    "  if (nextToken) __maaruLastToken = nextToken;",
    "  const err = parseMyshipError(text);",
    "  if (err) throw new Error(err);",
    "  const id = extractGmId(text, resp.url);",
    "  if (!id) throw new Error(\"fetch 成功但未找到 GM 賣場編號\");",
    "  return { id, url: resp.url };",
    "}",
    "",
    "async function createStore(store, plan) {",
    "  if (plan.mode === \"dom\") {",
    "    console.log(\"[MAARU] 改以 DOM 上架：\", store.name);",
    "    const id = await submitStoreDom(store, plan);",
    "    return { id, mode: \"dom\" };",
    "  }",
    "  try {",
    "    const r = await postStoreFetch(store, plan);",
    "    return { id: r.id, mode: \"fetch\" };",
    "  } catch (err) {",
    "    if (String(err && err.message || err).includes(\"DOM_ONLY\")) {",
    "      const id = await submitStoreDom(store, plan);",
    "      return { id, mode: \"dom\" };",
    "    }",
    "    console.warn(\"[MAARU] fetch 失敗，改 DOM 上架\", err);",
    "    const id = await submitStoreDom(store, plan);",
    "    return { id, mode: \"dom-fallback\" };",
    "  }",
    "}",
    "",
    ...
    "  console.log(\"[MAARU] POST URL\", getPostUrl());",
    "  getAllNamedFields(document, true).forEach((el, i) => {",
    "    console.log(i, el.name, el.type, el.value);",
    "  });",
    "  const plan = discoverProductPlan(root);",
    "  console.log(\"[MAARU] 商品欄位計畫\", plan);",
    "  console.log(\"[MAARU] 商品群組\", getProductIndexedGroup(root));",
    "};"
  ].join("\n");
  ...
  function buildMyshipConsoleScript_(ord, options) {
    ...
    lines.push("    const plan = discoverProductPlan();");
    lines.push("    console.log(\"[MAARU] 欄位模式\", plan.mode, plan.prefix || \"\");");
    lines.push("    const results = [];");
    lines.push("    for (let si = 0; si < stores.length; si++) {");
    lines.push("      const store = stores[si];");
    lines.push("      const created = await createStore(store, plan);");
    lines.push("      results.push({ store: store.name, id: created.id, mode: created.mode, ms: Date.now() - t0 });");
    lines.push("      if (si < stores.length - 1) {");
    lines.push("        if (String(created.mode || \"\").includes(\"dom\") && (location.pathname || \"\").toLowerCase().indexOf(\"/fast/\") < 0) {");
    lines.push("          location.href = MYSHIP_FAST_URL;");
    lines.push("          await sleep(2800);");
    lines.push("          await ensureProductRowVisible();");
    lines.push("        } else {");

...
var CONFIG = {
  ...
  // 會員主檔：獨立試算表「Memberist」分頁「會員名單」（姓名 ↔ 13 碼卡號）
  memberSpreadsheetId: "1CJiK7WjzbKtQnumL14H4dW0KkhdOd6RsyEN7-EU3qzw",
  memberSheetName: "會員名單"
};
  ...
  try {
    ...
    if (action === "member_list" || action === "member_upsert" || action === "member_delete" || action === "member_sync_from_orders" || action === "points_purge_card") {
      ...
      if (action === "member_sync_from_orders") {
        var syncResult = syncMembersFromOrders_(ss);
        out.members = getMembers(memberSheet);
        out.added = syncResult.added;
      ...
      if (action === "member_delete") {
        var delCard = normalizeMemberCardNo_(body.memberCardNo || body.card || "");
        if (!isValidMemberCardNo_(delCard)) {
          out.error = true;
      ...
      if (action === "member_upsert") {
        ...
        if (!member || typeof member !== "object") {
          out.error = true;
          out.message = "缺少 member 資料";
          return jsonOutput(out);
        }
        var upsertResult = upsertMemberRecord_(memberSheet, member, ss);
        if (upsertResult.error) {
          out.error = true;
          out.message = upsertResult.message;
          return jsonOutput(out);
        }
        out.member = upsertResult.member;
        out.message = upsertResult.created ? "已新增會員" : "已更新會員";
        out.created = upsertResult.created;
        return jsonOutput(out);
      }
    }
...
function orderKeyMap_(headers) {
  ...
  var aliases = [
    ...
    ["跨團轉入金額", "creditInAmount"],
    ["跨團轉入來源", "creditInFromOrderId", "creditInFromOrderIds"],
    ["跨團轉出金額", "creditOutAmount"],
    ["跨團轉出去向", "creditOutToOrderId"],
...
function normalizeOrderForSheet_(order) {
  ...
  var out = {
    ...
    creditInAmount: (o.creditInAmount != null && o.creditInAmount !== "") ? Number(o.creditInAmount) : "",
    creditInFromOrderId: (o.creditInFromOrderId != null) ? String(o.creditInFromOrderId).trim() : "",
    creditOutAmount: (o.creditOutAmount != null && o.creditOutAmount !== "") ? Number(o.creditOutAmount) : "",
    creditOutToOrderId: (o.creditOutToOrderId != null) ? String(o.creditOutToOrderId).trim() : "",
    trackingHistory: "",
    updated: "",
    itemsJson: ""
  };
...
function resolveMemberCardForOrderFromContext_(order, allOrders, ledger, ss) {
  var card = normalizeMemberCardNo_(order && order.memberCardNo);
  if (isValidMemberCardNo_(card)) return card;
  ss = ss || SpreadsheetApp.getActiveSpreadsheet();
  var rosterCard = lookupMemberCardFromRoster_(ss, order);
  if (isValidMemberCardNo_(rosterCard)) return rosterCard;
...
function enrichOrderForSheetWrite_(order, ss) {
  ...
  var card = resolveMemberCardForOrderFromContext_(order, allOrders, ledger, ss);
...
function syncPointsLedger(sheet, ledger, orders) {
  ...
  if (orders && orders.length) {
    ...
    merged = augmentLedgerFromAllOrders_(merged, orders);
  }
...
function orderHasPointsLedgerEntries_(ledger, orderId) {
  ...
  for (var i = 0; i < (ledger || []).length; i++) {
    var rec = ledger[i];
    if (normalizeOrderId_(rec.orderId) === oid) return true;
  }
  return false;
}
...
function augmentLedgerFromOrders_(ledger, card, allOrders) {
  ledger = (ledger || []).slice();
  card = normalizeMemberCardNo_(card);
  if (!isValidMemberCardNo_(card)) return ledger;
  var linkedNames = collectCustomerNamesForMemberCard_(allOrders, card, ledger);
  var today = todayStrForPoints_();
  for (var i = 0; i < (allOrders || []).length; i++) {
    var ord = allOrders[i];
    ...
    if (pe > 0 && !orderHasEarnLedgerEntryForCard_(ledger, oid, card, allOrders)) {
      ledger.push(repairPointLedgerRecordInPlace_({
        ...
        memberCardNo: normalizeMemberCardNo_(ord.memberCardNo) || card,
        type: "發放",
        points: pe,
        remaining: pe,
        expireDate: exp,
...
/** 將所有歷史訂單的紅利補入 ledger（後台同步試算表用） */
function augmentLedgerFromAllOrders_(ledger, allOrders) {
  allOrders = allOrders || [];
  var cards = {};
  for (var i = 0; i < allOrders.length; i++) {
    var c = normalizeMemberCardNo_(allOrders[i] && allOrders[i].memberCardNo);
    if (isValidMemberCardNo_(c)) cards[c] = true;
  }
  for (var card in cards) {
    if (!Object.prototype.hasOwnProperty.call(cards, card)) continue;
    ledger = augmentLedgerFromOrders_(ledger, card, allOrders);
  }
  return ledger;
}

/** 查詢／同步時：依歷史訂單補寫缺漏的紅利分頁列 */
function syncMissingLedgerEntriesForMemberCard_(ss, card, allOrders) {
  card = normalizeMemberCardNo_(card);
...
function preparePublicPointsContext_(ss) {
  ss = ss || SpreadsheetApp.getActiveSpreadsheet();
  var allOrders = parsePublicOrdersFast_(ss);
  // 公開查詢勿傳 ss：resolveOrderPointsFields_(order, ss) 會逐筆重讀訂單＋紅利分頁（極慢、易逾時）
  var resolved = [];
  for (var i = 0; i < allOrders.length; i++) {
    resolved.push(resolveOrderPointsFields_(allOrders[i], null));
  }
  var pointsSheet = getPointsSheet(ss);
  var ledger = pointsSheet ? getPointsLedger(pointsSheet) : [];
...
function orderEffectiveShippingPublic_(ord) {
  ...
  // 有關聯訂單且未填運費：偏向併箱副單，不預設加 38
  var linked = String(ord && ord.linkedOrderIds || "").trim();
  if (linked) return 0;
  return 38;
}

function isCancelledShipStatusPublic_(status) {
  var st = String(status || "").trim();
  return st === "缺貨取消" || st === "已取消" || st.indexOf("缺貨") >= 0 && st.indexOf("取消") >= 0;
}

function orderItemLineAmountPublic_(it) {
  var qty = Math.max(0, Math.floor(Number(it && (it.qty != null ? it.qty : it.quantity)) || 0));
  if (!qty) qty = 1;
  var price = Number(it && it.price);
  if (isNaN(price)) return 0;
  return Math.max(0, Math.ceil(price * qty));
}

function orderCancelledAmountPublic_(ord) {
  var items = (ord && Array.isArray(ord.items)) ? ord.items : [];
  var sum = 0;
  for (var i = 0; i < items.length; i++) {
    var st = String(items[i] && (items[i].shipStatus || items[i].itemStatus) || "");
    if (isCancelledShipStatusPublic_(st)) sum += orderItemLineAmountPublic_(items[i]);
  }
...
function orderProductNetPublic_(ord) {
  var items = (ord && Array.isArray(ord.items)) ? ord.items : [];
  var fromItems = 0;
  var hasPricedActive = false;
  for (var i = 0; i < items.length; i++) {
    var st = String(items[i] && (items[i].shipStatus || items[i].itemStatus) || "");
    if (isCancelledShipStatusPublic_(st)) continue;
    var price = Number(items[i] && items[i].price);
    if (!isNaN(price)) {
      hasPricedActive = true;
      fromItems += orderItemLineAmountPublic_(items[i]);
    }
  }
  var sub = hasPricedActive ? fromItems : (Number(ord && ord.subtotal) || 0);
  if (!hasPricedActive && items.length) {
    // 有品項但無單價時：小計扣掉取消金額
    sub = Math.max(0, (Number(ord && ord.subtotal) || 0) - orderCancelledAmountPublic_(ord));
  }
...
function orderAmountDuePublic_(ord) {
  ...
  var creditIn = Number(ord && ord.creditInAmount) || 0;
  if (isNaN(creditIn) || creditIn < 0) creditIn = 0;
  var gross = Math.max(0, Math.ceil(net + ship - creditIn));
...
/**
 * 「工作表1」的商品內容常只有單一品項，歷史訂單的品項(JSON)才是完整清單。
 * 品項數較多時以歷史訂單為主，並沿用工作表1已知的出貨狀態與圖片。
 */
function mergeShopItemsIntoStatusResult_(result, shopOrd) {
  var shopItems = (shopOrd && Array.isArray(shopOrd.items)) ? shopOrd.items : [];
  if (!shopItems.length) return result;
  var current = Array.isArray(result.items) ? result.items : [];
  if (shopItems.length <= current.length) return result;

  var prevByName = {};
  for (var i = 0; i < current.length; i++) {
    var key = String((current[i] && current[i].name) || "").trim();
    if (key) prevByName[key] = current[i];
  }

  var merged = [];
  for (var j = 0; j < shopItems.length; j++) {
    var mapped = mapPublicStatusItem_(shopItems[j]);
    if (!mapped.name) continue;
    var prev = prevByName[mapped.name];
    if (prev) {
      if (prev.itemStatus) mapped.itemStatus = prev.itemStatus;
      if (prev.itemStatusCode) mapped.itemStatusCode = prev.itemStatusCode;
      if (prev.image) mapped.image = prev.image;
    }
    merged.push(mapped);
  }
  ...
  result.items = merged;
  result.itemSummary = buildPublicItemSummary_(merged);
  result.product = buildOrderProductText_(shopOrd, merged);
  result.status = derivePublicTrackingStatus_(shopOrd, merged);
  return result;
}
...
function applyOrderStatusMoneyFields_(result, ord, items) {
  ...
  result.cancelledAmount = orderCancelledAmountPublic_(ord);
  result.creditInAmount = Math.max(0, Math.floor(Number(ord.creditInAmount) || 0));
  result.creditInFromOrderId = String(ord.creditInFromOrderId || "").trim();
  result.creditOutAmount = Math.max(0, Math.floor(Number(ord.creditOutAmount) || 0));
  result.creditOutToOrderId = String(ord.creditOutToOrderId || "").trim();
...
function sanitizePublicOrder_(ord) {
  ...
  return {
    ...
    cancelledAmount: orderCancelledAmountPublic_(ord),
    creditInAmount: Math.max(0, Math.floor(Number(ord && ord.creditInAmount) || 0)),
    creditInFromOrderId: String(ord && ord.creditInFromOrderId != null ? ord.creditInFromOrderId : "").trim(),
    creditOutAmount: Math.max(0, Math.floor(Number(ord && ord.creditOutAmount) || 0)),
    creditOutToOrderId: String(ord && ord.creditOutToOrderId != null ? ord.creditOutToOrderId : "").trim(),
...
function getCustomerOrdersPublic_(params) {
  ...
  for (var j = 0; j < matched.length; j++) {
    publicOrders.push(sanitizePublicOrder_(resolveOrderPointsFields_(matched[j], null)));
  }
  var totalDue = 0;
  var activeCount = 0;
...
function mapPublicStatusItemCode_(label) {
  ...
  if (isCancelledShipStatusPublic_(text)) return "cancelled";
...
function mapPublicStatusItem_(it) {
  ...
  var price = (o.price != null && o.price !== "" && !isNaN(Number(o.price))) ? Number(o.price) : null;
  var code = mapPublicStatusItemCode_(label);
  return {
    name: name || "商品",
    image: image,
    qty: qty > 0 ? qty : 1,
    price: price,
    itemStatus: code === "cancelled" ? "缺貨取消" : label,
    itemStatusCode: code
  };
}
...
function buildPublicItemSummary_(items) {
  ...
  for (var i = 0; i < total; i++) {
    if (items[i].itemStatusCode === "cancelled") cancelled++;
    else if (items[i].itemStatusCode === "shipped") shipped++;
  }
  return {
    total: total,
    shipped: shipped,
    cancelled: cancelled,
    pending: Math.max(0, total - shipped - cancelled)
  };
}
...
function derivePublicTrackingStatus_(ord, items) {
  ...
  var active = 0;
  for (var i = 0; i < (items || []).length; i++) {
    if (items[i].itemStatusCode === "cancelled") continue;
    active++;
    if (items[i].itemStatusCode === "shipped") shipped++;
  }
  if (active > 0 && shipped === active) return "已出貨";
  if (shipped > 0) return "部分已出貨";
  if (items && items.length && active === 0) return "缺貨取消";
...
function assertOrderStatusCardAccess_(params, memberCardNoOnOrder) {
  var card = resolvePublicMemberCardParam_(params);
  if (!isValidMemberCardNo_(card)) {
    return {
      error: true,
      message: "請使用會員卡號查詢交易紀錄後，再查看訂單明細"
    };
  }
  var orderCard = normalizeMemberCardNo_(memberCardNoOnOrder || "");
  if (!isValidMemberCardNo_(orderCard) || orderCard !== card) {
...
function getOrderStatusPublic_(params) {
  ...
  if (sheet1Result && sheet1Result.error === false) {
    if (shopOrd) {
      ...
      mergeShopItemsIntoStatusResult_(sheet1Result, shopOrd);
      applyOrderStatusMoneyFields_(sheet1Result, shopOrd, sheet1Result.items);
    }
    var gate1 = assertOrderStatusCardAccess_(params, sheet1Result.memberCardNo);
  ...
  var gate = assertOrderStatusCardAccess_(params, ord.memberCardNo);
  if (gate) return gate;
...
/** 會員名單試算表（獨立 Memberist；留空 memberSpreadsheetId 則用 ProductManagement2 本分頁） */
function getMemberSpreadsheet_(fallbackSs) {
  var id = (CONFIG.memberSpreadsheetId || "").toString().trim();
  if (id) {
    try {
      return SpreadsheetApp.openById(id);
    } catch (err) {
      Logger.log("[getMemberSpreadsheet_] " + err);
      throw new Error(
        "無法開啟會員名單試算表。請將試算表共用「編輯者」給部署 Apps Script 的 Google 帳號：" + id
      );
    }
  }
  return fallbackSs || SpreadsheetApp.getActiveSpreadsheet();
}
...
function getMemberSheet(fallbackSs) {
  var ss = getMemberSpreadsheet_(fallbackSs);
  var name = (CONFIG.memberSheetName || "會員名單").toString().trim();
  var sheet = ss.getSheetByName(name);
  if (!sheet && name !== "會員名單") {
    sheet = ss.getSheetByName("會員名單");
  }
  if (!sheet) {
    var all = ss.getSheets();
    for (var i = 0; i < all.length; i++) {
      var n = (all[i].getName() || "").toString().trim();
      if (n === name || n === "會員名單" || n.indexOf("會員") >= 0) {
        sheet = all[i];
        break;
      }
    }
...
function getStandardMemberHeaders_() {
  return [
    "會員卡號",
    "姓名",
    "電話",
...
function ensureMemberHeaderRow_(sheet) {
  var headers = getStandardMemberHeaders_();
  if (sheet.getLastRow() < 1) {
    sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
    return;
  }
...
function memberKeyMap_(headers) {
  var map = {};
  var aliases = [
    ["會員卡號", "memberCardNo", "memberCard"],
...
function normalizeMemberRecord_(rec) {
  var r = rec || {};
  return {
    memberCardNo: normalizeMemberCardNo_(r.memberCardNo || r.memberCard || ""),
    ...
    createdAt: normalizeSheetDateValue_(r.createdAt) || "",
    updatedAt: normalizeSheetDateValue_(r.updatedAt) || ""
  };
}

function buildRowFromMemberRecord_(rec) {
  var norm = normalizeMemberRecord_(rec);
  var headers = getStandardMemberHeaders_();
  var keyMap = memberKeyMap_(headers);
  var row = [];
  for (var c = 0; c < headers.length; c++) {
    var key = keyMap[c];
    var val = "";
    if (key && norm[key] !== undefined && norm[key] !== null && norm[key] !== "") val = norm[key];
    if (key === "memberCardNo" && norm.memberCardNo) val = "'" + norm.memberCardNo;
    row.push(val);
  }
  return row;
}
...
function getMembers(sheet) {
  ...
  for (var r = 1; r < data.length; r++) {
    ...
    if (cardCol >= 0) {
      ...
      if (isValidMemberCardNo_(dispCard)) {
        obj.memberCardNo = dispCard;
        empty = false;
      }
    }
    if (!isValidMemberCardNo_(normalizeMemberCardNo_(obj.memberCardNo))) {
      var scanned = extractMemberCardFromRowCells_(row, dispRow);
      if (isValidMemberCardNo_(scanned)) {
        obj.memberCardNo = scanned;
        empty = false;
    ...
    if (empty || !isValidMemberCardNo_(normalizeMemberCardNo_(obj.memberCardNo))) continue;
    list.push(normalizeMemberRecord_(obj));
  }
  list.sort(function(a, b) {
...
function findMemberRowByCard_(sheet, card) {
  ...
  var cardCol = findMemberCardColumnIndex_(headers);
  if (cardCol < 0) cardCol = 0;
  var display = sheet.getRange(2, cardCol + 1, lastRow - 2 + 1, 1).getDisplayValues();
  var values = sheet.getRange(2, cardCol + 1, lastRow - 2 + 1, 1).getValues();
  for (var r = 0; r < display.length; r++) {
    var c = normalizeMemberCardNo_(display[r][0] || values[r][0]);
    if (c === card) return r + 2;
  }
  return 0;
}

function scoreMemberRecordCompleteness_(rec) {
  var s = 0;
  if (!rec) return 0;
...
function pickRicherMemberRecord_(a, b) {
  if (!a) return b;
  if (!b) return a;
  return scoreMemberRecordCompleteness_(a) >= scoreMemberRecordCompleteness_(b) ? a : b;
}

function lookupMemberCardFromRoster_(ss, order) {
  if (!order) return "";
  var sheet = getMemberSheet(ss);
  var members = getMembers(sheet);
  var n = normalizeCustomerNameForPoints_(order.customerName);
  var p = normalizePhoneForPoints_(order.phone);
  var l = normalizeLineIdForPoints_(order.lineId);
  var direct = normalizeMemberCardNo_(order.memberCardNo);
  if (isValidMemberCardNo_(direct)) {
    for (var i = 0; i < members.length; i++) {
      if (normalizeMemberCardNo_(members[i].memberCardNo) === direct) return direct;
    }
  }
  for (var j = 0; j < members.length; j++) {
    var m = members[j];
    if (String(m.status || "").trim() === "停用") continue;
    if (n && normalizeCustomerNameForPoints_(m.customerName) === n) {
      return normalizeMemberCardNo_(m.memberCardNo);
    }
  }
  for (var k = 0; k < members.length; k++) {
    var m2 = members[k];
    if (String(m2.status || "").trim() === "停用") continue;
    if (p && normalizePhoneForPoints_(m2.phone) === p) {
      return normalizeMemberCardNo_(m2.memberCardNo);
    }
  }
  for (var x = 0; x < members.length; x++) {
    var m3 = members[x];
    if (String(m3.status || "").trim() === "停用") continue;
    if (l && normalizeLineIdForPoints_(m3.lineId) === l) {
      return normalizeMemberCardNo_(m3.memberCardNo);
    }
  }
  return "";
}

function upsertMemberFromOrder_(ss, order) {
...
function upsertMemberRecord_(sheet, member, ss) {
  ...
  var created = rowNum <= 0;
  if (created) {
    member.createdAt = member.createdAt || now.slice(0, 10);
  } else {
    var prev = null;
    for (var j = 0; j < existing.length; j++) {
      if (normalizeMemberCardNo_(existing[j].memberCardNo) === card) {
        prev = existing[j];
        break;
      }
    }
  ...
  return { error: false, member: member, created: created };
}

function deleteMemberByCard_(sheet, card) {
  var rowNum = findMemberRowByCard_(sheet, card);
...
function syncMembersFromOrders_(ss) {
  ss = ss || SpreadsheetApp.getActiveSpreadsheet();
  var sheet = getMemberSheet(ss);
  ...
  for (var i = 0; i < orders.length; i++) {
    ...
    var nKey = normalizeCustomerNameForPoints_(name);
    if (nKey) {
      if (!nameToCards[nKey]) nameToCards[nKey] = {};
      nameToCards[nKey][card] = true;
    }
    if (!byCard[card]) {
      byCard[card] = {
        memberCardNo: card,
      ...
      byCard[card] = normalizeMemberRecord_(pickRicherMemberRecord_(byCard[card], {
        memberCardNo: card,
        customerName: name,
        phone: ord.phone || "",
        lineId: ord.lineId || "",
  ...
  Object.keys(nameToCards).forEach(function(nk) {
    var cards = Object.keys(nameToCards[nk]);
    if (cards.length > 1) {
      conflicts.push({ customerName: nk, memberCardNos: cards });
    }
  });
